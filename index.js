...
function checkCollision(rock) {
  ...
  if (rockTop > 360) {
    ...
    if (false === /*** Think about it -- what's happening here?
               ...
               *    and the rock's right edge is > the DODGER's right edge */
(rockLeftEdge < dodgerLeftEdge && rockRightEdge > dodgetLeftEdge) ||
...
function createRock(x) {
  ...
  function moveRockDown() {
    ...
    // * If a rock collides with the DODGER, we should call endGame()*/
     ...
     else if (positionToInteger(rock.style.top) > 0) {
     ...
     else if (0 >= positionToInteger(rock.style.top)) {
       rock.remove();
     }
   }
...
function endGame() {
  ...
  document.querySelectorAll('.rock').forEach(e => e.parentNode.removeChild(e));
  moveDodger().removeEventListener;
  moveDodgerLeft().removeEventListener;
  moveDodgerRight().removeEventListener;
  alert("YOU LOSE!");
}
